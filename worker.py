...
def _link_volume_custom_nodes() -> list[str]:
    """Symlink /runpod-volume/ComfyUI/custom_nodes/* into /ComfyUI/custom_nodes.
    ...
    linked: list[str] = []
    if not os.path.isdir(vol) or not os.path.isdir(dst):
        return linked
    for name in os.listdir(vol):
        if name.startswith((".", "__")):
            continue
        src_path = os.path.join(vol, name)
        ...
        try:
            if os.path.islink(target) or not os.path.exists(target):
                ...
                linked.append(name)
                print(f"[worker] linked volume custom node: {name}", flush=True)
        except OSError as exc:
            print(f"[worker] failed to link {name}: {exc}", flush=True)
    return linked


def _ensure_volume_nodes_loaded() -> None:
    """Link volume custom nodes; restart ComfyUI if any new packages appeared."""
    linked = _link_volume_custom_nodes()
    if not linked:
        return
    # Only restart if ComfyUI is up and missing these types would matter —
    # a restart picks up newly linked packages.
    try:
        print(f"[worker] Restarting ComfyUI to load volume nodes: {linked}", flush=True)
        node_installer.restart_comfyui()
    except Exception as exc:
        print(f"[worker] WARNING: ComfyUI restart after volume node link failed: {exc}", flush=True)
